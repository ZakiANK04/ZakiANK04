# 👋 Hello, I'm Ahcene Zakaria AOUANOUK

🎓 **Industrial Engineering Student (Data Science & AI)**
📍 National Polytechnic School of Algiers (ENP)
🚀 Passionate about Artificial Intelligence, Optimization & Industrial Innovation

---

## 👨‍💻 Who am I?

I’m a State Engineer student in **Industrial Engineering – Data Science & AI** at ENP Algiers 

I love building intelligent systems that solve **real-world industrial problems**, combining:

* 🧠 Artificial Intelligence
* 📊 Data Science
* ⚙️ Optimization & Complex Systems
* 💡 Strategic Decision-Making

I enjoy transforming raw data into actionable insights and scalable solutions.

---

## 🔭 What am I currently working on?

* 🤖 AI-powered industrial data preprocessing platforms
* 📈 Predictive modeling & scoring systems
* 🧾 NLP projects (CV analysis, document processing)
* 🧠 Machine Learning research & applied AI
* 🌱 Smart agriculture & demand prediction systems

Always experimenting. Always building.

---

## 🛠️ Tools & Technologies I Use

### 💻 Programming Languages

```
Python | C++ | SQL | Java | MATLAB | LaTeX
```

### 🤖 Data Science & AI

* Neural Networks
* NLP (Natural Language Processing)
* Classification & Regression Models
* Data Cleaning & Filtering
* Database Management

### 🧰 Software & Tools

* Power BI
* Docker
* Git & GitHub
* Google Colab
* Excel / Word / PowerPoint
* Adobe Suite (Illustrator, Premiere Pro, Photoshop, After Effects)

---

## 🏆 Projects & Achievements

### 🥈 2nd Place – Algeria 2.0 (Agridata Insight 2025)

Built **AgroVisor**, an intelligent agricultural advisory system:

* 🌾 Crop selection
* 💰 Price prediction
* 📊 Yield forecasting
* ⚠️ Risk assessment

### 🥈 2nd Place – DevCamp 2025

Developed an AI-powered web platform to predict demand for Agri-Food products.

### 🤖 Machine Learning Projects

* Face Matching Application
* CV Analysis & Evaluation using NLP

---

## 🏢 Professional Experience

### 📊 Data Science & Database Intern – SAA Assurances (2025)

* Designed employee scoring model using claims data

### 🧠 AI Research Intern – KABAS (2025)

* Improved predictive models for drilling fluid formulation
* Built AI-driven industrial data preprocessing tools

### 🏭 Intern – NAFTAL (2024)

* Exposure to industrial systems, HSE standards & enterprise tools

---

## 🎯 Beyond Tech

* 🎨 Graphic Design & Video Editing
* 📷 Photography & Videography
* 🥋 Judo
* ⚽ Football
* 🤝 Active in multiple engineering & innovation clubs (VIC, IEEE, IEC)

---

## 🌍 Languages

* 🇩🇿 Arabic (Native)
* 🇫🇷 French (B2)
* 🇬🇧 English (C1)

---

## 📫 How to Reach Me?

📧 **Email:** [zzaouanouk@gmail.com](mailto:zzaouanouk@gmail.com)
📱 **Phone:** +213 672 97 15 75
💼 **LinkedIn:** [www.linkedin.com/in/ahcene-zakaria-aouanouk](http://www.linkedin.com/in/ahcene-zakaria-aouanouk)

---

## ✨ Motto

> “Turning data into decisions. Turning ideas into impact.”

---
